...
function [A_n,A_p,B_n,B_p,C_n,C_p,D_n,D_p,varargout] = spm_plant_obs_mats(p, varargin)
import casadi.*
if nargin == 2
    T = varargin{1};
else
...
alpha_n = D_s_n / (p.R_s_n * p.delta_r_n)^2;
alpha_p = D_s_p / (p.R_s_p * p.delta_r_p)^2;

% Block matrices
if isa(T, 'casadi.MX')
    M1_n = MX.zeros(p.Nr-1, p.Nr-1);
    M1_p = MX.zeros(p.Nr-1, p.Nr-1);
else
    M1_n = zeros(p.Nr-1);
    M1_p = zeros(p.Nr-1);
end
...
if isa(T, 'casadi.MX')
    M2_n = MX.zeros(p.Nr-1,2);
    M2_p = MX.zeros(p.Nr-1,2);
else
    M2_n = zeros(p.Nr-1,2);
    M2_p = zeros(p.Nr-1,2);
end
...
% N2 = diag([-1,1]);
% 
% N3_n = [0; (p.delta_r_n * p.R_s_n)/(D_s_n * p.Faraday * p.a_s_n * p.Area * p.L_n)];
% N3_p = [0; -(p.delta_r_p * p.R_s_p)/(D_s_p * p.Faraday * p.a_s_p * p.Area * p.L_p)];
...
N3_n = [0; -(2*p.delta_r_n * p.R_s_n)/(D_s_n)];
N3_p = [0; -(2*p.delta_r_p * p.R_s_p)/(D_s_p)];
