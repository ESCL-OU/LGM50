...
function [An,Bn,Ap,Bp,varargout] = c_s_mats(p, varargin)
...
D_s_n = p.D_s_n0 * exp(p.E.Dsn/p.R*(1/p.T_ref - 1/T));
D_s_p = p.D_s_p0 * exp(p.E.Dsp/p.R*(1/p.T_ref - 1/T));
...
b_vecn = symsubsnum(p.R_s_n, D_s_n); %New Jan 10, 2014
a_vecn = symsubsden(p.R_s_n, D_s_n); %New Jan 10, 2014

b_vecp = symsubsnum(p.R_s_p, D_s_p); %New Jan 10, 2014
a_vecp = symsubsden(p.R_s_p, D_s_p); %New Jan 10, 2014
...
Vn = [-(p.R_s_n^4*(9*sqrt(2429)-457))/(381150*D_s_n^2), (p.R_s_n^4*(9*sqrt(2429)+457))/(381150*D_s_n^2), 1;...
       p.R_s_n^2*(sqrt(2429)-63)/(2310*D_s_n), -p.R_s_n^2*(sqrt(2429)+63)/(2310*D_s_n), 0;...
       1, 1, 0];
Vp = [-(p.R_s_p^4*(9*sqrt(2429)-457))/(381150*D_s_p^2), (p.R_s_p^4*(9*sqrt(2429)+457))/(381150*D_s_p^2), 1;...
       p.R_s_p^2*(sqrt(2429)-63)/(2310*D_s_p), -p.R_s_p^2*(sqrt(2429)+63)/(2310*D_s_p), 0;...
       1, 1, 0];
...
An_normalized_D=An/D_s_n; %added by Federico for sensitivity analysis (equivalent to having D_s_n=1)
...
Ap_normalized_D=Ap/D_s_p; %added by Federico for sensitivity analysis (equivalent to having D_s_p=1)
