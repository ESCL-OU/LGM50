...
[D_en] = electrolyteDe(p, c_en, T);
[D_es] = electrolyteDe(p, c_es, T);
[D_ep] = electrolyteDe(p, c_ep, T);
...
if(nargout > 4)
    ...
    B1(end) = (Volt + I*(p.D_psp(end)-p.D_psn(1))) / (p.rho_avg*p.C_p);
end
...
k_n = p.k_n0 * exp(p.E.kn/p.R*(1/p.T_ref - 1/T));
k_p = p.k_p0 * exp(p.E.kp/p.R*(1/p.T_ref - 1/T));

di0dcssn = k_n*c_en.*(p.c_s_n_max - 2*c_ss_n) ./ ...
            (2 * sqrt(c_en.*c_ss_n.*(p.c_s_n_max - c_ss_n)));
di0dcssp = k_p*c_ep.*(p.c_s_p_max - 2*c_ss_p) ./ ...
            (2 * sqrt(c_ep.*c_ss_p.*(p.c_s_p_max - c_ss_p)));

di0dcen = k_n*sqrt(c_en.*c_ss_n.*(p.c_s_n_max - c_ss_n)) ./ (2*c_en);
di0dcep = k_p*sqrt(c_ep.*c_ss_p.*(p.c_s_p_max - c_ss_p)) ./ (2*c_ep);
